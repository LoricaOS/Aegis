...
typedef struct aegis_process {
    ...
    /* 1 iff admin_session was granted by the FIRST-BOOT exception rather than
     * by a verified credential. Credential-backed elevation deliberately
     * survives exec (that is the sudo model: an elevated shell runs elevated
     * commands). First-boot elevation must NOT — `configure` runs before any
     * credential exists, and g_first_boot is latched for the whole boot, so
     * without this a setup flow that ends by launching the desktop instead of
     * rebooting left the entire session tree admin-elevated. Cleared with
     * admin_session at exec; see sys_execve. */
    uint32_t      admin_session_firstboot;
...
/* Find a user process by PID. CALLER MUST HOLD sched_lock and must not use the
 * result after dropping it — PCBs have no refcount and sys_waitpid's reaper
 * frees them. Returns NULL if not found. See the comment on the definition. */
aegis_process_t *proc_find_by_pid_locked(uint32_t pid);

/* Existence check only; takes sched_lock itself. Use when you never need to
 * touch the PCB. */
int proc_pid_exists(uint32_t pid);
